# This file contains code modifications of the Stable Baselines3 buffers.py file

# We have copied the file and marked the required modifications for the SUFT method (marked as # SUFT CHANGE)
...
class ReplayBuffer(BaseBuffer):
    ...
    dones: np.ndarray
    timeouts: np.ndarray
	old_values_to_store: np.ndarray # SUFT CHANGE
    ...
    def __init__(
        ...
        # SUFT CHANGE (keep only one of the below changes for DQN/SAC)
        # SUFT CHANGE for DQN methods
        self.old_values = np.zeros((self.buffer_size, self.n_envs, 1), dtype=np.float32)
        # SUFT CHANGE for SAC
        ...
        if psutil is not None:
            total_memory_usage: float = (
                # SUFT CHANGE (keep only one of the below changes for DQN/SAC)
                # SUFT CHANGE for DQN methods
                self.observations.nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes + self.old_values.nbytes
                # SUFT CHANGE for SAC
                self.observations.nbytes + self.actions.nbytes + self.rewards.nbytes + self.dones.nbytes + self.old_values1.nbytes + self.old_values2.nbytes
                # SUFT CHANGE until here
            )
    ...
    def add(
		...
		old_values_to_store, # SUFT CHANGE
        ...
        # SUFT CHANGE for SAC
        first_old_values = old_values_to_store[0].detach().numpy()
        second_old_values = old_values_to_store[1].detach().numpy()
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        data = (
            ...
            # SUFT CHANGE (keep only one of the below changes for DQN/SAC)
            # SUFT CHANGE for DQN methods
            self.old_values[batch_inds, env_indices], 
            # SUFT CHANGE for SAC
            self.old_values1[batch_inds, env_indices],  
            self.old_values2[batch_inds, env_indices],  
            # SUFT CHANGE until here
        )
