...
from stable_baselines3.common.utils import safe_mean, should_collect_more_steps, obs_as_tensor # SUFT CHANGE
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _store_transition(
        ...
        old_values_to_store = th.gather(self.policy.q_net(obs_tensor).cpu(), dim=1, index=th.from_numpy(buffer_action).long())
        # SUFT CHANGE for SAC
        self.policy.critic.set_training_mode(False)
        obs_tensor = obs_as_tensor(self._last_original_obs, self.device)
        old_values_to_store = self.policy.critic(obs_tensor, th.from_numpy(buffer_action))
